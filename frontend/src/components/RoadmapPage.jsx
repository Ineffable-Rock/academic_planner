...
import { 
  ...
  Lightbulb, 
  Share2 as GraphIcon, 
  GitMerge as FullGraphIcon // ✅ Added new icon
} from 'lucide-react';
...
// ==================== Main Roadmap Page ====================
const RoadmapPage = ({ careerPath, onBack, onViewGraph, onViewFullRoadmap }) => {
    ...
    const fetchRoadmap = async () => {
      try {
        const response = await fetch(
          `http://127.0.0.1:5000/api/career-paths/${careerPath.id}/roadmap`
        );
  ...
  return (
    <div>
      <button 
        onClick={onBack} 
        className="flex items-center gap-2 text-zinc-500 hover:text-blue-500 transition-colors mb-6"
      >
        <ArrowLeft size={20} />
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          <h1 className="text-4xl font-bold text-zinc-800">
            {careerPath.name} Roadmap
          </h1>
          <p className="text-zinc-500 mt-2">
            Your step-by-step guide from beginner to advanced.
          </p>
        </div>
